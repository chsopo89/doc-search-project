...
def build_tfidf(df, column="content_clean"):
  vector = TfidfVectorizer(max_features=5000, min_df=2, stop_words="english")
  matrix = vector.fit_transform(df[column])
...
def main():
    ...
    preprocess_data(df)
    k = 3
    matrix, vector = build_tfidf(df, "content_clean")
    print(tfidf_search(df, "python list comprehension", vector, matrix, k)) 
    print("=== 기본 방식 ===")
    run_evaluation(df, vector, matrix, k)
    analyze_failures(df, vector, matrix, k)
    matrix2, vector2 = build_tfidf(df, "content_weighted")
    print("=== 개선 방식 ===")
    run_evaluation(df, vector2, matrix2, k)
...
def run_evaluation(df, vector, matrix, k):
  run_evaluation = [
...
def analyze_failures(df, vector, matrix, k):
  analyze_failures = [
